...
protected:
    ...
    struct vertex {
        ...
        vertex(T n) : node(move(n)), pre(-1), g(-1.0), flag(false) {}
    };
    ...
    virtual double _compute_step_cost(int v1, int v2) { return _compute_dist(v1, v2); }
    virtual double _compute_heuristic(int v1, int v2) { return _compute_dist(v1, v2); }
...
void Astar<T>::_solve(int start, int end) {
    ...
    double f = _compute_heuristic(start, end);
    s.g = 0;
    ...
    while (!q.empty()) {
        ...
        for (const int &u : eight_successors) {
            double step_cost = _compute_step_cost(v.second, u);
            double gu = vertices[v.second].g + step_cost;
            double fu = gu + epilode * _compute_heuristic(u, end);

            if (!vertices[u].flag && (vertices[u].g < 0 || gu < vertices[u].g)) {
                vertices[u].g = gu;
